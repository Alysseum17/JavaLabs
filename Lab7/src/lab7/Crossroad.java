...
public class Crossroad implements Serializable {
    ...
    public static class TrafficStats implements Serializable {
        ...
        public double getDangerIndex() {
            ...
            return (double) accidentsPerYear / (vehiclesPerHour * 365.0) * 100_000;
        }

        @Override
        public String toString() {
            return String.format(
                    "Авто/год: %d, Пішоходи/год: %d, ДТП/рік: %d (небезпека: %.2f)",
                    vehiclesPerHour,
                    pedestriansPerHour,
                    accidentsPerYear,
                    getDangerIndex()
    ...
    public Crossroad(String crossroadName, String[] streets, double latitude, double longitude, boolean regulated) {
    ...
    public Crossroad(String name, String[] streets, double lat, double lon, boolean regulated, int vehiclesPerHour, int accidents, int pedestrians) {
    ...
    public void setStats(TrafficStats s) {
        this.stats = s;
    }
    ...
    public boolean containsStreet(String streetName) {
        for (String s : streets) {
            if (s != null && s.equalsIgnoreCase(streetName)) {
                return true;
            }
        }
        return false;
    ...
    public static Crossroad findMostDangerous(Crossroad[] arr) {
        ...
        for (Crossroad c : arr) {
            if (c != null && c.getDangerIndex() > most.getDangerIndex()) {
                most = c;
            }
        }
    ...
    public String toString() {
        return String.format(
                ...
                id,
                crossroadName,
                regulated ? "регульоване" : "нерегульоване"
        );
    }
